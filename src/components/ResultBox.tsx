...
type Props = {
  ...
  age: string;
  metadata: any;
  // developmentalNotes は不要（result から内部パース）
...
export default function ResultBox({ result, color, type, age, metadata }: Props) {
  ...
  const mainText = sections[0] || result;
  const devMemo = sections.find((s) => s.includes("発達の視点")) || "";
  const checkPoints = sections.find((s) => s.includes("確認チェック")) || "";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/records", {
        ...
        body: JSON.stringify({ type, age, title, content: mainText, metadata }),
      });
    ...
    <div className="mt-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      {devMemo && (
        <div className="mt-3">
          ...
          {notesExpanded && (
            <div
              className="mt-2 p-4 rounded-2xl text-sm leading-relaxed text-[#3A3A3A] whitespace-pre-wrap"
      ...
      {/* ── セクション3：確認チェックポイント（小さく常時表示） ── */}
      {checkPoints && (
        <div className="mt-3 px-1">
          <div className="text-[11px] text-[#94A3AE] leading-relaxed whitespace-pre-wrap font-medium">
            {checkPoints}
          </div>
        </div>
      )}
